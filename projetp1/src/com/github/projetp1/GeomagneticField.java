...
/**
 * Estimates magnetic field at a given point on Earth, and in particular, to compute the magnetic
 ...
 * Agency. More details about the model can be found at <a
 * href="http://www.ngdc.noaa.gov/geomag/WMM/DoDWMM.shtml"
 * >http://www.ngdc.noaa.gov/geomag/WMM/DoDWMM.shtml</a>. This class currently uses WMM-2010 which
 * is valid until 2015, but should produce acceptable results for several years after that. Future
 * versions of Android may use a newer version of the model.
 */
public class GeomagneticField
{
	...
	static private final float[][] G_COEFF = new float[][] { { 0.0f }, { -29496.6f, -1586.3f },
			{ -2396.6f, 3026.1f, 1668.6f }, { 1340.1f, -2326.2f, 1231.9f, 634.0f },
	...
	static private final float[][] H_COEFF = new float[][] { { 0.0f }, { 0.0f, 4944.4f },
			{ 0.0f, -2707.7f, -576.1f }, { 0.0f, -160.2f, 251.9f, -536.6f },
	...
	static private final float[][] DELTA_G = new float[][] { { 0.0f }, { 11.6f, 16.5f },
			{ -12.1f, -4.4f, 1.9f }, { 0.4f, -4.1f, -2.9f, -7.7f },
			{ -1.8f, 2.3f, -8.7f, 4.6f, -2.1f }, { -1.0f, 0.6f, -1.8f, -1.0f, 0.9f, 1.0f },
	...
	static private final float[][] DELTA_H = new float[][] { { 0.0f }, { 0.0f, -25.9f },
			{ 0.0f, -22.5f, -11.8f }, { 0.0f, 7.3f, -3.9f, -2.6f },
			{ 0.0f, 1.1f, 2.7f, 3.9f, -0.8f }, { 0.0f, 0.4f, 1.8f, 1.2f, 4.0f, -0.6f },
	...
	static private final long BASE_TIME = new GregorianCalendar(2010, 1, 1).getTimeInMillis();
	 ...
	 * @param timeMillis
	 *            Time at which to evaluate the declination, in milliseconds since January 1, 1970.
	 *            (approximate is fine -- the declination changes very slowly).
	 */
	public GeomagneticField(float gdLatitudeDeg, float gdLongitudeDeg, float altitudeMeters,
			long timeMillis)
	{
		...
		LegendreTable legendre = new LegendreTable(MAX_N - 1,
		...
		for (int n = 1; n < MAX_N; n++)
		{
			for (int m = 0; m <= n; m++)
			{
				...
				gcX += relativeRadiusPower[n + 2] * (g * cosMLon[m] + h * sinMLon[m])
						* legendre.mPDeriv[n][m] * SCHMIDT_QUASI_NORM_FACTORS[n][m];

				// Negative derivative with respect to longitude, divided by
				// radius.
				gcY += relativeRadiusPower[n + 2] * m * (g * sinMLon[m] - h * cosMLon[m])
						* legendre.mP[n][m] * SCHMIDT_QUASI_NORM_FACTORS[n][m] * inverseCosLatitude;

				// Negative derivative with respect to radius.
				gcZ -= (n + 1) * relativeRadiusPower[n + 2] * (g * cosMLon[m] + h * sinMLon[m])
						* legendre.mP[n][m] * SCHMIDT_QUASI_NORM_FACTORS[n][m];
			}
		}
	...
	/**
	 * @return The declination of the horizontal component of the magnetic field from true north, in
	 *         degrees (i.e. positive means the magnetic field is rotated east that much from true
	 *         north).
	 */
	...
	/**
	 * @return The inclination of the magnetic field in degrees -- positive means the magnetic field
	 ...
	 * @return Geocentric latitude (i.e. angle between closest point on the equator and this point,
	 *         at the center of the earth.
	 */
	private void computeGeocentricCoordinates(float gdLatitudeDeg, float gdLongitudeDeg,
	...
	/**
	 * Utility class to compute a table of Gauss-normalized associated Legendre functions
	 * P_n^m(cos(theta))
	 */
	static private class LegendreTable
	{
		 ...
		 * @param thetaRad
		 *            Returned functions will be Gauss-normalized P_n^m(cos(thetaRad)), with
		 *            thetaRad in radians.
		 */
			...
			for (int n = 1; n <= maxN; n++)
			{
				...
				for (int m = 0; m <= n; m++)
				{
					...
					else
					{
						...
						mPDeriv[n][m] = -sin * mP[n - 1][m] + cos * mPDeriv[n - 1][m] - k
								* mPDeriv[n - 2][m];
					}
				}
			}
	...
	/**
	 * Compute the ration between the Gauss-normalized associated Legendre functions and the Schmidt
	 * quasi-normalized version. This is equivalent to
